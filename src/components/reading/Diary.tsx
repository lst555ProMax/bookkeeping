...
const Reading: React.FC = () => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (modeSwitcher) {
        ...
        if (hasUnsavedDiary && hasUnsavedQuickNote) {
          message = '当前有未保存的阅读日记和速记，是否继续当前操作？\n\n';
        } else if (hasUnsavedDiary) {
          message = '当前有未保存的阅读日记，是否继续当前操作？\n\n';
  ...
  const handleDeleteDiary = (id: string, silent: boolean = false) => {
    // 如果不是静默删除，需要确认
    if (!silent) {
      const confirmed = window.confirm('确定要删除这篇阅读日记吗？\n\n删除后将无法恢复！');
      if (!confirmed) return;
    }

...
const ReadingNotebook = forwardRef<ReadingNotebookRef, ReadingNotebookProps>(({
  selectedDate,
  onDateChange,
  currentTheme,
  onThemeChange,
  ...
  const extensions = useMemo(() => [
    ...
    Placeholder.configure({
      placeholder: '记录你的阅读感悟（按Ctrl+Enter保存）',
    }),
  ], []);
      ...
      <div className="notebook__page" style={{ backgroundColor: currentTheme }}>
        <div className="notebook__header">
          ...
          <div className="notebook__actions">
            <button className="action-icon-btn" onClick={onNew} title="新建书记">
              ➕
            </button>
            ...
            <BubbleMenu className="bubble-menu" editor={editor}>
              ...
              <div className="bubble-menu__row" key={`text-color-${selectionUpdate}`}>
                {TEXT_COLORS.map((color) => {
                  ...
                  // 调试：如果当前颜色存在但未匹配，输出日志
                  if (currentColor && !isActive && normalizedCurrentColor) {
...
ReadingNotebook.displayName = 'ReadingNotebook';

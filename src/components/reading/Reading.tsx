...
import ReadingNotebook, { ReadingNotebookRef } from './ReadingNotebook/ReadingNotebook';
...
const Reading: React.FC = () => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (modeSwitcher) {
        ...
        if (hasUnsavedDiary && hasUnsavedQuickNote) {
          message = '当前有未保存的书记和摘抄，是否继续当前操作？\n\n';
        } else if (hasUnsavedDiary) {
          message = '当前有未保存的书记，是否继续当前操作？\n\n';
        } else if (hasUnsavedQuickNote) {
          message = '当前有未保存的摘抄，是否继续当前操作？\n\n';
        }
  ...
  const handleExportQuickNotes = () => {
    try {
      ...
      const message = `确定导出摘抄吗？\n\n摘抄：${filtered.length} 条${quickNotesSearch.trim() ? `（已筛选）` : ''}`;
      
      if (window.confirm(message)) {
        exportQuickNotesOnly(filtered);
        toast.success('摘抄数据导出成功！');
      }
    } catch (error) {
  ...
  const handleImportQuickNotes = async (file: File) => {
    ...
    try {
      ...
      const message = `导入完成！\n新增 ${result.imported} 条摘抄，跳过 ${result.skipped} 条重复记录`;
  ...
  const handleDeleteAllQuickNotes = () => {
    try {
      ...
      setQuickNotes([]);
      toast.success(`已删除 ${count} 条摘抄`);
      } catch (error) {
      console.error('删除所有摘抄失败:', error);
  ...
  const handleSaveDiary = async () => {
    ...
    // 保存后让编辑器失去焦点
    setTimeout(() => {
      readingNotebookRef.current?.blurEditor();
    }, 100);
  };

...
import { getDiaryEntryNumber } from '@/utils/music';
...
const MusicList: React.FC<MusicListProps> = ({
  ...
  const handleExport = async (entry: DiaryEntry, format: 'txt' | 'doc' | 'pdf' | 'md') => {
    ...
    try {
      if (format === 'txt') {
        ...
        downloadFile(blob, `乐记_${formatDate(entry.date)}_${getEntryNumber(entry)}.txt`);
      } else if (format === 'md') {
        // 导出为markdown（包含图片）
        let mdContent = `# ${formatDate(entry.date)} #${getEntryNumber(entry)}\n\n`;
        ...
        downloadFile(blob, `乐记_${formatDate(entry.date)}_${getEntryNumber(entry)}.md`);
      } else if (format === 'doc') {
        ...
        downloadFile(blob, `乐记_${formatDate(entry.date)}_${getEntryNumber(entry)}.doc`);
      } else if (format === 'pdf') {
        ...
        const htmlContent = `
          ...
          <head>
            <meta charset='utf-8'>
            <title>乐记 - ${formatDate(entry.date)}</title>
            <style>
              @media print {
  ...
  const handleDeleteAll = () => {
    ...
    const confirmed = window.confirm(
      `确定要删除所有 ${diaryEntries.length} 篇乐记吗？\n\n此操作无法撤销！建议先导出备份。`
    );
  ...
  return (
    <div className="diary-list">
      <div className="diary-list__header">
        <h3 className="diary-list__title">🎵 乐记 ({diaryEntries.length})</h3>
        {onSearchContentChange && (
      ...
      <div className="diary-list__items" ref={itemsContainerRef}>
        ...
        {diaryEntries.length === 0 && (
          <div className="empty-state">
            ...
            <p>还没有乐记哦</p>
            <p className="empty-state__hint">在中间写下你的第一篇乐记吧！</p>
          </div>
        )}
      </div>
    </div>

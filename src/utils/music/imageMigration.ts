...
import { loadMusicEntries, saveMusicEntries } from './storage';
import { saveImageToIndexedDB } from '@/utils/common/imageStorage';

/**
 * 迁移单个乐记的图片到 IndexedDB
 */
const migrateEntryImage = async (entry: DiaryEntry): Promise<DiaryEntry> => {
  ...
  if (entry.image && entry.image.trim() !== '') {
    try {
      ...
      console.error(`迁移乐记 ${entry.id} 的图片失败:`, error);
      // 迁移失败，保留原 entry
      return entry;
    }
  }
...
export const migrateAllImagesToIndexedDB = async (): Promise<{
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.error(`迁移乐记 ${entry.id} 失败:`, error);
  ...
  if (migrated > 0 || failed > 0) {
    saveMusicEntries(migratedEntries);
  }

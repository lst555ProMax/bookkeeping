      ...
      reader.onload = (event) => {
        try {
          ...
          if ('musicLyrics' in legacyData && legacyData.musicLyrics !== undefined && legacyData.musicLyrics !== null) {
            ...
            // 既没有 musicLyrics 也没有 quickNotes
            throw new Error('无效的数据格式：缺少musicLyrics数组或quickNotes数组');
          }
          
          const lyrics = (legacyData.musicLyrics || legacyData.quickNotes || []) as QuickNote[];
          
          if (!Array.isArray(lyrics)) {
            throw new Error('无效的数据格式：musicLyrics或quickNotes必须是数组');
          }
      ...
      reader.onload = async (event) => {
        try {
          ...
          if (legacyData.musicEntries && Array.isArray(legacyData.musicEntries)) {
            ...
            // 既没有 musicEntries 也没有 diaryEntries
            throw new Error('无效的数据格式：缺少musicEntries数组或diaryEntries数组');
          }
          
          const entries = (legacyData.musicEntries || legacyData.diaryEntries || []) as DiaryEntry[];
          
          if (!Array.isArray(entries)) {
            throw new Error('无效的数据格式：musicEntries或diaryEntries必须是数组');
          }
          ...
          for (const entry of newEntries) {
            ...
            if (entry.image && typeof entry.image === 'string' && entry.image.trim() !== '') {
              try {
                ...
                console.error(`保存图片到 IndexedDB 失败 (乐记 ${entry.id}):`, error);
